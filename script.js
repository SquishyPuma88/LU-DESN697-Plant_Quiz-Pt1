document.addEventListener("DOMContentLoaded", (event) => {
  const basePath = "https://raw.githubusercontent.com/SquishyPuma88/Plant_Images/main/";

  // Define lists for quiz data
  const genusList = ["Pachystegia", "Carmichaelia", "Rhopalostylis", "Astelia", "Ligularia", "Pratia", "Myosotidium", "Apodasmia", "Phormium", "Gaura", "Chionochloa", "Betula", "Acer", "Anemanthele", "Pittosporum", "Stachys", "Penstemon", "Lavendula", "Echinacea", "Sedum", "Arthropodium", "Dianella", "Pimelea", "Catalpa", "Melicytus", "Podocarpus", "Teucridium", "Coprosma", "Phormium", "Olearia", "Rubus", "Pittosporum", "Haloregis", "Libertia", "Poa", "Fuscospora", "Dacrydium", "Solanum", "Sceranthus", "Muehlenbeckia", "Euphorbia", "Fuscospora", "Agathis", "Pseudowintera", "Coprosma", "Sophora", "Cordyline", "Pseudopanax", "Pseudopanax"];
  
  const speciesList = ["insignis", "stevensonii", "sapida", "chathamica", "dentata", "angulata", "hortensia", "similis", "cookianum", "lindheimeri", "flavicans", "pendula", "platanoides", "lessoniana", "tenuifolium", "byzantina", "scrophulariaceae", "angustifolia", "purpurea", "spectabile", "cirratum", "nigra", "prostrata", "bignonioides", "ramiflorus", "nivalis", "parvifolium", "rugosa", "tenax", "cheesemanii", "cissoides", "tenuifolium", "erecta", "peregrinans", "cita", "cliffortioides", "cupressinum", "laciniatum", "biflorus", "axillaris", "glauca", "fusca", "australis", "colorata", "propinqua", "microphylla", "australis", "crassifolius", "ferox"];
...
const growingConditionsList = [
  ...
  function getShuffledOptions(correctAnswer, list) {
    ...
    const options = [correctAnswer, ...filteredList.slice(0, 3)];
  ...
  function showQuestion(index) {
    ...
    shuffleArray(questionData.options.genus);
    shuffleArray(questionData.options.species);
    shuffleArray(questionData.options.commonName);
    shuffleArray(questionData.options.growingConditions);
    quizContainer.innerHTML = `
      <img src="${questionData.image}" alt="Plant Image">
      <h2>${questionData.question}</h2>
      <div>
        <label>Genus:</label>
        ${questionData.options.genus.map(option => `<button class="option" data-type="genus" data-answer="${option}">${option}</button>`).join('')}
      </div>
      <div>
        <label>Species:</label>
        ${questionData.options.species.map(option => `<button class="option" data-type="species" data-answer="${option}">${option}</button>`).join('')}
      </div>
      <div>
        <label>Common Name:</label>
        ${questionData.options.commonName.map(option => `<button class="option" data-type="commonName" data-answer="${option}">${option}</button>`).join('')}
      </div>
      <div>
        <label>Growing Conditions:</label>
        ${questionData.options.growingConditions.map(option => `<button class="option" data-type="growingConditions" data-answer="${option}">${option}</button>`).join('')}
      </div>
    `;
    ...
    document.querySelectorAll('.option').forEach(button => {
      button.addEventListener('click', (event) => {
        ...
        if (answer === questionData.correctAnswers[type]) {
          score++;
        ...
        highlightCorrectAnswers(questionData, type);
      });
    });
  }

  function highlightCorrectAnswers(questionData, type) {
    document.querySelectorAll(`.option[data-type="${type}"]`).forEach(button => {
      if (button.getAttribute('data-answer') === questionData.correctAnswers[type]) {
  ...
  function showFinalScore() {
    ...
    quizContainer.innerHTML = `
      <h2>Final Score</h2>
      <p>You got ${score} out of ${totalQuestions} correct (${percentage.toFixed(2)}%).</p>
  ...
  nextButton.addEventListener('click', () => {
    if (seenQuestions.size < quizData.length) {
      currentQuestionIndex = getRandomQuestionIndex();
      showQuestion(currentQuestionIndex);
    } else {
      showFinalScore();
  ...
  previousButton.addEventListener('click', () => {
    // Implement logic to go back to the previous question if needed
  });

  submitButton.addEventListener('click', () => {
    // Add logic to check answers and display results
  });
});
